...
public class DrinkShopController {
    ...
    private void onAddNewIngred() {
        ...
        try {
            ...
            showError("Cantitate ingredient invalida. Introdu un numar valid.");
            return;
        }
    ...
    private void onAddNewReteta() {
        int nextRetetaId = service.getAllRetete().stream()
                .mapToInt(Reteta::getId)
                .max()
                .orElse(0) + 1;
        Reteta r = new Reteta(nextRetetaId, new ArrayList<>(newRetetaList));
        service.addReteta(r);
        newRetetaList.clear();
        initData();
    }
    ...
    private void onFinalizeOrder() {
        if (currentOrderItems.isEmpty()) {
            showError("Comanda nu poate fi finalizata fara produse.");
            return;
        }
